...
describe("TimeInterval", () => {
  describe("constructor", () => {
    ...
    it("throws on invalid single argument", () => {
      assertThrows(() => new TimeInterval("not a date" as unknown as TjTime), Error, "Illegal argument 1");
    });

    it("throws on invalid start type", () => {
      const end = TjTime.fromSeconds(200);
      assertThrows(() => new TimeInterval("not a date" as unknown as TjTime, end), Error, "Interval start must be a date");
    });

    it("throws on invalid end type", () => {
      const start = TjTime.fromSeconds(100);
      assertThrows(() => new TimeInterval(start, "not a date" as unknown as TjTime), Error, "Interval end must be a date");
    });

    it("throws on too many arguments", () => {
      const start = TjTime.fromSeconds(100);
      const end = TjTime.fromSeconds(200);
      assertThrows(() => new TimeInterval(start, end, TjTime.fromSeconds(300)), Error, "Too many arguments");
    });
  });
