...
describe("ScoreboardInterval", () => {
  describe("constructor", () => {
    it("creates from single ScoreboardInterval copy", () => {
      ...
      const start = 2;
      const end = 5;
      const original = new ScoreboardInterval(
        sbStart,
        slotDuration,
        start,
        end,
      ...
      assertThrows(
        () =>
          new ScoreboardInterval(
            "not a scoreboard interval" as unknown as ScoreboardInterval,
          ),
        Error,
        "Illegal argument 1",
      );
    ...
    it("throws on invalid sbStart type", () => {
      assertThrows(
        () => new ScoreboardInterval("not a date", 60, 2,),
        Error,
        "sbStart must be a date",
      );
    });
    ...
    it("throws on invalid slotDuration type", () => {
      const sbStart = TjTime.fromSeconds(100,);
      assertThrows(
        () => new ScoreboardInterval(sbStart, "not an integer", 2,),
        Error,
        "slotDuration must be an integer",
      );
    });
      ...
      assertThrows(
        () =>
          new ScoreboardInterval(
            sbStart,
            slotDuration,
            "not an integer or date",
          ),
        Error,
        "start must be an integer or TjTime",
      );
    });
    ...
    it("throws on invalid end type (non-integer, non-TjTime)", () => {
      ...
      assertThrows(
        ...
        Error,
        "end must be an integer or TjTime",
      );
    });
    ...
    it("throws on too many arguments", () => {
      ...
      const slotDuration = 60;
      assertThrows(
        () => new ScoreboardInterval(sbStart, slotDuration, 2, 5, 10,),
        Error,
        "Too many arguments",
      );
    });
  ...
  describe("to_s", () => {
    it("formats as startDate - endDate", () => {
      ...
      assertEquals(
        iv.to_s(),
        `${expectedStart.to_s()} - ${expectedEnd.to_s()}`,
      );
    });
