...
describe("Scoreboard", () => {
  describe("constructor", () => {
    ...
    it("throws on invalid startDate type", () => {
      assertThrows(() => new Scoreboard("not a date" as unknown as Date, new Date(), 60), Error, "startDate must be a Date");
    });

    it("throws on invalid endDate type", () => {
      assertThrows(() => new Scoreboard(new Date(), "not a date" as unknown as Date, 60), Error, "endDate must be a Date");
    });

    it("throws on invalid resolution type", () => {
      assertThrows(() => new Scoreboard(new Date(), new Date(), "not a number" as unknown as number), Error, "resolution must be a positive integer");
    });

    it("throws on non-positive resolution", () => {
      assertThrows(() => new Scoreboard(new Date(), new Date(), 0), Error, "resolution must be a positive integer");
    });

    it("throws when endDate is before startDate", () => {
      assertThrows(() => new Scoreboard(new Date("2026-01-02"), new Date("2026-01-01"), 60), Error, "endDate must be after startDate");
    });
  });
  ...
  describe("idxToDate", () => {
    ...
    it("throws on negative index without forceIntoProject", () => {
      ...
      const resolution = 60;
      const sb = new Scoreboard<string>(start, end, resolution);
      assertThrows(() => sb.idxToDate(-1), Error, "Index -1 is out of scoreboard range");
    });
