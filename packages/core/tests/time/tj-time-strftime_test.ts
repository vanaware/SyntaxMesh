...
describe("TjTime.to_s", () => {
  describe("uses original seconds (this.seconds % 60) for :%S decision", () => {
    ...
    it("excludes :%S when original seconds == 0", () => {
      ...
      assertEquals(result.match(/^\d{4}-\d{2}-\d{2}-\d{2}:\d{2}-[+-]\d{4}$/) !== null, true); // format: YYYY-MM-DD-HH:MM-+HHMM
    });
  });
  ...
  describe("default format", () => {
    it("format is %Y-%m-%d-%H:%M-%z when seconds == 0", () => {
      ...
      assertEquals(result.match(/^\d{4}-\d{2}-\d{2}-\d{2}:\d{2}-[+-]\d{4}$/) !== null, true);
    });
    ...
    it("format is %Y-%m-%d-%H:%M:%S-%z when seconds != 0", () => {
      ...
      assertEquals(result.match(/^\d{4}-\d{2}-\d{2}-\d{2}:\d{2}:\d{2}-[+-]\d{4}$/) !== null, true);
    });
