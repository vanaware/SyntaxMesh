...
class TestDependencyListAttribute extends DependencyListAttribute {
  constructor(property: { id: string; name: string; }, type: AttributeDefinition<{ task: { fullId: string } }[]>, container: MockContainer) {
    super(property, type, container);
  }
}

class TestTaskDepListAttribute extends TaskDepListAttribute {
  constructor(property: { id: string; name: string; }, type: AttributeDefinition<{ task: { fullId: string }; onEnd: boolean }[]>, container: MockContainer) {
    super(property, type, container);
  }
}
...
describe("DependencyAttributes", () => {
  ...
  describe("TaskDepListAttribute", () => {
    let type: AttributeDefinition<{ task: { fullId: string }; onEnd: boolean }[]>;
