...
console.log(
  "sameTimeNextMonth Jan31:",
  JSON.stringify(partsOf(m.sameTimeNextMonth(),),),
);

const y = TjTime.fromParts(2024, 2, 29, 12, 0, 0,);
console.log(
  "sameTimeNextYear Feb29:",
  JSON.stringify(partsOf(y.sameTimeNextYear(),),),
);

const ci = TjTime.fromParts(2026, 1, 15, 14, 30, 45,);
console.log(
  "collectIntervals empty:",
  JSON.stringify(
    ci.collectIntervals(ci.addSeconds(0,), 3600,).map((iv,) => ({
      start: iv.start.to_s(undefined, "UTC",),
      end: iv.end.to_s(undefined, "UTC",),
    })),
  ),
);
console.log(
  "collectIntervals single:",
  JSON.stringify(
    ci.collectIntervals(ci.addSeconds(3600,), 3600,).map((iv,) => ({
      start: iv.start.to_s(undefined, "UTC",),
      end: iv.end.to_s(undefined, "UTC",),
    })),
  ),
...
console.log(
  "collectIntervals multi:",
  JSON.stringify(
    ci.collectIntervals(ci.addSeconds(7200,), 3600,).map((iv,) => ({
      start: iv.start.to_s(undefined, "UTC",),
      end: iv.end.to_s(undefined, "UTC",),
    })),
  ),
...
console.log(
  "beginOfWeek Sunday start:",
  JSON.stringify(partsOf(w.beginOfWeek(false,),),),
);
console.log(
  "beginOfWeek Monday start:",
  JSON.stringify(partsOf(w.beginOfWeek(true,),),),
);
