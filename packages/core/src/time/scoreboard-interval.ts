...
export class ScoreboardInterval extends Interval<number> {
  ...
  constructor(...args: unknown[]) {
    if (args.length === 1) {
      ...
      if (!(sbStart instanceof TjTime)) {
        throw new TjArgumentError(`sbStart must be a date, not a ${typeof sbStart}`);
      }
      if (typeof slotDuration !== 'number' || !Number.isInteger(slotDuration)) {
        throw new TjArgumentError(`slotDuration must be an integer, not a ${typeof slotDuration}`);
      }
      ...
      if (typeof start !== 'number' || !Number.isInteger(start)) {
        throw new TjArgumentError(`start must be an integer or TjTime, not ${typeof start}`);
      }
      super(start, start);
      ...
      if (typeof end !== 'number' || !Number.isInteger(end)) {
        throw new TjArgumentError(`end must be an integer or TjTime, not ${typeof end}`);
      }
      super(start, end);
      this.sbStart = sbStart;
      this.slotDuration = slotDuration;
  ...
  to_s(): string {
    return this.indexToDate(this.start).to_s() + ' - ' + this.indexToDate(this.end).to_s();
  }
}
