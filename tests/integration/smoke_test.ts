/**
 * @syntaxmesh/tests/integration/smoke_test.ts
 *
...
describe("smoke", () => {
  ...
  it("deve ser possível carregar o módulo parser", async () => {
    ...
    assert(typeof parserMod.PARSER_VERSION === "string", "parser deve exportar PARSER_VERSION");
  });

  it("deve ser possível carregar o módulo report", async () => {
    const reportMod = await import(join(ROOT, "packages/report/src/mod.ts"));
    assert(typeof reportMod.REPORT_VERSION === "string", "report deve exportar REPORT_VERSION");
  });

  it("deve ser possível carregar o módulo storage", async () => {
    const storageMod = await import(join(ROOT, "packages/storage/src/mod.ts"));
    assert(typeof storageMod.STORAGE_VERSION === "string", "storage deve exportar STORAGE_VERSION");
  });
